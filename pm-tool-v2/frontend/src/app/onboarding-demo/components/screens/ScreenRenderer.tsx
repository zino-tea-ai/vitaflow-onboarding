...
export function ScreenRenderer() {
  ...
  const config = getScreenConfig(currentStep)
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/1146cc51-3fe3-46a3-9e1a-4801e1a50de0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ScreenRenderer.tsx:52',message:'ScreenRenderer render',data:{currentStep,configExists:!!config,configId:config?.id,configType:config?.type},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'A,B'})}).catch(()=>{});
  // #endregion
  
  if (!config) {
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/1146cc51-3fe3-46a3-9e1a-4801e1a50de0',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ScreenRenderer.tsx:56',message:'CONFIG IS UNDEFINED - CRASH',data:{currentStep},timestamp:Date.now(),sessionId:'debug-session',hypothesisId:'B'})}).catch(()=>{});
    // #endregion
    return (
