...
async def step_organize_desktop(
    ...
    if state.desktop_organized:
        if status_callback:
            await status_callback("⚡ 桌面已整理，跳过此步骤")
        return {
            "status": "skipped",
    ...
    if status_callback:
        await status_callback("📁 正在使用 UFO 整理桌面文件...")
    ...
    try:
        ...
        if status_callback:
            await status_callback("  → 调用 UFO 执行桌面整理任务...")
        ...
        if result.status == TaskStatus.SUCCESS:
            ...
            if status_callback:
                await status_callback("✅ UFO 完成桌面整理")
            return {
                "status": "completed",
...
async def _step_organize_desktop_fallback(
    ...
    for file in files_to_organize:
        try:
            ...
            if status_callback:
                await status_callback(f"  → 移动: {file.name}")
        except Exception as e:
            logger.error(f"Failed to move {file.name}: {e}")
    
    state.mark_desktop_organized()
    
    result_msg = f"已整理 {len(moved_files)} 个文件到 {YC_FOLDER_NAME}/" if moved_files else "没有需要整理的文件"
    if status_callback:
...
async def step_read_document(
    ...
    """
    if status_callback:
        await status_callback("📖 正在读取产品文档...")
        await asyncio.sleep(0.3)  # Visual delay
        await status_callback("  → 打开 PITCH_CONTEXT.md")
    ...
    try:
        ...
        if doc_content:
            ...
            if status_callback:
                await status_callback(f"⚠️ 未找到文档，将使用预设答案")
            return {
                "status": "completed",
        ...
        if status_callback:
            await status_callback(f"⚠️ 读取失败: {e}，将使用预设答案")
        return {
...
async def step_read_yc_questions(
    ...
    """
    if status_callback:
        await status_callback("🔍 正在读取 YC 表单问题...")
        await status_callback("  → 连接到已 Hook 的 Chrome...")
    ...
    try:
        ...
        if not yc_target:
            if status_callback:
                await status_callback("❌ 未找到 YC 申请页面")
        ...
        async with websockets.connect(ws_url) as ws:
            ...
            if questions:
                if status_callback:
                    ...
                    await status_callback("✅ YC 问题读取完成")
                ...
                if status_callback:
                    await status_callback("⚠️ 未找到表单问题")
                return {
                    "status": "completed",
        ...
        if status_callback:
            await status_callback(f"❌ 读取 YC 问题失败: {e}")
        return {
            "status": "failed",
            "message": str(e),
            "questions": [],
...
async def step_generate_answer(
    ...
    """
    if status_callback:
        await status_callback("🤖 正在使用 AI 生成答案...")
...
## Your Answer:"""
        ...
        if status_callback:
            ...
            await status_callback("✅ AI 答案生成完成")
        ...
        if status_callback:
            await status_callback(f"❌ AI 生成答案失败: {e}")
        return {
            "status": "failed",
            "message": str(e),
            "answer": None,
...
async def step_fill_form(
    ...
    if status_callback:
        await status_callback("✍️ 正在填写 YC 表单...")
        await status_callback("  → 连接到已 Hook 的 Chrome...")
    ...
    try:
        ...
        # JavaScript to find and fill the textarea
            ...
            if eval_result.get("success"):
                if status_callback:
                    await status_callback(f"  → ✅ 已填写到 {eval_result.get('field', 'textarea')}")
                    await status_callback("✅ 表单填写完成!")
                return {
                    "status": "completed",
                ...
                if status_callback:
                    await status_callback(f"⚠️ 填写失败: {error}")
        ...
        # #endregion
        if status_callback:
            await status_callback(f"❌ CDP 填写失败: {e}")
        return {
            "status": "failed",
            "message": str(e),
        }
...
async def _step_fill_form_browser_use_fallback(
    ...
    try:
        if not browser_session or not browser_session.is_started:
            if status_callback:
                await status_callback("⚠️ 浏览器会话未激活")
            return {
                "status": "failed",
        ...
        if not success:
            ...
            if status_callback:
                await status_callback("  → 尝试使用键盘输入...")
            ...
            try:
                ...
                success = True
                if status_callback:
                    await status_callback("✅ 通过键盘输入成功")
            except Exception as e:
        ...
        if status_callback:
            await status_callback(f"❌ 填写失败: {e}")
        return {
            "status": "failed",
            "message": str(e),
        }
...
async def step_send_whatsapp(
    ...
    """
            if status_callback:
        await status_callback(f"📱 正在使用 UFO 发送 WhatsApp 消息到 {contact}...")
    ...
    try:
        ...
        if status_callback:
            await status_callback("  → UFO 正在执行 WhatsApp 发送任务...")
        
        result = await execute_desktop_task(task)
        
        if result.status == TaskStatus.SUCCESS:
            if status_callback:
                await status_callback("✅ UFO 成功发送 WhatsApp 消息")
            return {
                "status": "completed",
...
async def _step_send_whatsapp_fallback(
    ...
    try:
        ...
        if status_callback:
            await status_callback(f"  → 使用 pyautogui 输入消息: {message}")
        ...
        if status_callback:
            await status_callback("✅ WhatsApp 消息已发送")
        ...
        if status_callback:
            await status_callback("⚠️ pyautogui 未安装，跳过 WhatsApp 发送")
        return {
            "status": "skipped",
        ...
        if status_callback:
            await status_callback(f"❌ WhatsApp 发送失败: {e}")
        return {
            "status": "failed",
            "message": str(e),
        }
...
async def execute_yc_workflow(
    ...
    if status_callback:
        ...
        await status_callback(f"🚀 NogicOS 智能 YC Workflow - 执行 #{state.execution_count + 1}")
        await status_callback("=" * 50)
        await asyncio.sleep(0.3)
    ...
    try:
        ...
        if not product_doc:
            if status_callback:
                await status_callback("❌ 无法读取产品文档，工作流终止")
            return {
                "success": False,
        ...
        if not yc_questions:
            ...
            if status_callback:
                await status_callback("⚠️ 未找到表单问题，使用默认问题")
            yc_questions = [{
                "index": 0,
                "question": "What is your company going to make?",
        ...
        if answer_result["status"] == "failed" or not answer_result.get("answer"):
            if status_callback:
                await status_callback("❌ AI 无法生成答案，工作流终止")
            return {
                "success": False,
        ...
        if not confirmed:
            if status_callback:
                await status_callback("❌ 用户取消了工作流")
            return {
                "success": False,
        ...
        if status_callback:
            await status_callback("\n" + "=" * 50)
            await status_callback("✅ 智能工作流执行完成!")
            await status_callback("=" * 50)
            await status_callback(f"📝 AI 生成并填写的答案:")
