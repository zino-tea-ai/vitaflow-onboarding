...
└─────────────────────┬───────────────────┬───────────────────┘
                      ...
                      ▼                   ▼
┌─────────────────────────────────────────────────────────────┐
│                Python Core - Hive Engine                     │
│  ┌──────────┐  ┌──────────────┐  ┌──────────────┐          │
│  │  Router  │  │ LangGraph    │  │  Knowledge   │          │
│  │          │◄─┤   Agent      │◄─┤    Store     │          │
│  └──────────┘  └──────────────┘  └──────────────┘          │
│                       │                    │                │
│                       ▼                    ▼                │
│              ┌──────────────┐     ┌──────────────┐         │
│              │  Playwright  │     │ Cloud Sync   │         │
│              │  Controller  │     │   Service    │         │
│              └──────────────┘     └──────────────┘         │
└─────────────────────────────────────────────────────────────┘
                      │                    │
...
| 层级 | 技术 | 理由 |
|------|------|------|
| 框架 | Electron 33 + Vite | 最新稳定版，热更新 |
| UI | React 19 + TypeScript | 组件化，类型安全 |
| 样式 | Tailwind CSS v4 | 原子化，高性能 |
| 状态 | Zustand | 轻量，无样板代码 |
| IPC | electron-trpc | 类型安全的进程通信 |
...
| 层级 | 技术 | 理由 |
|------|------|------|
| Agent 框架 | LangGraph 1.0 | 状态机，记忆持久化，human-in-the-loop |
| 浏览器控制 | Playwright | 跨浏览器，CDP 原生支持 |
| 知识存储 | LangMem + SQLite | 向量搜索，本地优先 |
| API | FastAPI + WebSocket | 异步，实时通信 |
| LLM | Claude Opus 4.5 | 最强推理能力 |
...
```
User ──► Webview ──► Recorder ──► Knowledge
  │         │           │            │
  │ 正常操作  │ CDP事件流   │ 识别模式    │ 保存trajectory
  │         │           │            │
  └─────────┴───────────┴────────────┘
               自动去噪、匿名化
```
...
发现 Bug
    │
    ▼
运行 health check ──► 模块异常? ──► 运行契约测试 ──► 修复该模块
    │
    ▼
全部正常?
    │
    ▼
运行集成测试 ──► 查看 trace 日志 ──► 修复问题
```

### 关键文件

- 健康检查：`engine/health/checks.py`
- 契约测试：`tests/contracts/`
- 追踪日志：`engine/observability/tracer.py`
...
1. **优先级**：先做 Electron 客户端还是先完善 Python 引擎？
2. **被动学习**：用户操作默认自动学习，还是需要手动开启？
3. **云端同步**：第一版是否需要云端知识共享，还是先做本地？

---

## 实施待办

- [ ] 用 LangGraph 重写 Hive Agent，实现状态图和记忆持久化
- [ ] 搭建 Electron + React + Vite 客户端框架
- [ ] 实现 Electron webview 与 Playwright 的 CDP 桥接
- [ ] 实现被动学习系统：CDP 事件捕获 + 模式识别
- [ ] 实现知识库本地存储和云端同步
- [ ] 准备 Demo 场景和数据
