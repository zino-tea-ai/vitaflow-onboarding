...
export function getAriaSelected(element: Element): boolean | null {
...
export function getAriaChecked(element: Element): boolean | 'mixed' | null {
...
export function getAriaPressed(element: Element): boolean | 'mixed' | null {
...
export function getAriaExpanded(element: Element): boolean | null {
...
export function getAriaDisabled(element: Element): boolean | null {
...
function isNativelyDisabled(element: Element): boolean | null {
  // https://www.w3.org/TR/html-aam-1.0/#html-attribute-state-and-property-mappings
...
function belongsToDisabledFieldSet(element: Element | null): boolean | null {
  if (!element)
    return null;
  //   return false;
...
function hasExplicitAriaDisabled(element: Element | undefined): boolean | null {
  if (!element)
    return null;
  // return false;
...
/**
 * Code hereafter is written by Michael Y. Fatemi
 */

type ElementProperties = Pick<AXTree, 'bounding_box' | 'name' | 'properties' | 'role'>;
function getElementInfo(element: Element, includeHidden: boolean): ElementProperties {
  ...
  return result as ElementProperties;
}
...
interface AXTree {
  ...
  children: AXTree[],
  locator_parts: { selector: string, parameters: any }[],
}
...
function _populateLocatorParts(axtree: AXTree, locatorParts: LocatorPart[]) {
  ...
  for (let childIndex = 0; childIndex < children.length; childIndex++) {
    ...
    const newLocatorsForChild: LocatorPart[] = [];
...
function _getAccessibilityTree(element: Element, includeHidden: boolean, idCounter: number): AXTree | AXTree[] {
  let children: Element[] = [...element.children];

  const id = idCounter;
  idCounter += 1;

  const childrenResults: AXTree[] = [];
  ...
  if (element === document.body) {
    return {
      ...getElementInfo(element, includeHidden),
      id,
      role: "document",
      name: document.title,
  ...
  const skipNode = (
    ...
    (name.length === 0) && (
      (Object.keys(getElementInfo(element, includeHidden).properties).length === 0 && [
        "generic",
        "img",
        "list",
        "strong",
...
function getAccessibilityTree(): AXTree {
  let idCounter = 0;
