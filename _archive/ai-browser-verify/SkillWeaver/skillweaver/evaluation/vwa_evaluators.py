...
from skillweaver.evaluation.vwa_helper_functions import (
    PseudoPage,
    get_query_text,
...
import skillweaver.evaluation.vwa_image_utils as image_utils

Action = dict
Trajectory = tuple[list[State], list[Action]]
...
class Evaluator(object):
    ...
    @staticmethod
    def get_last_state(trajectory: Trajectory) -> State:
...
class StringEvaluator(Evaluator):
    ...
    async def __call__(
        self, trajectory: Trajectory, config_file: Path | str, page: Page
    ) -> float:
        with open(config_file, "r") as f:
            configs = json.load(f)

        last_action = self.get_last_action(trajectory)
        pred_orig = self.clean_answer(last_action["terminate_with_result"])

        score = 1.0
        for approach, value in configs["eval"]["reference_answers"].items():
            match approach:
                case "exact_match":
                    score *= self.exact_match(ref=value, pred=pred_orig)
                case "required_values":
                    required_values = value
                    assert isinstance(required_values, list)
                    pred = NumericEvaluator.str_2_int(pred_orig)
                    if pred is None:
                        score = 0.0
                    else:
                ...
                case "must_include":
                    ...
                    for must_value in value:
                        ...
                        score *= any(
                            [self.must_include(ref=v, pred=pred_orig) for v in value_or]
                        )
                case "must_exclude":
                    assert isinstance(value, list)
                    for must_excl_value in value:
                        score *= self.must_exclude(ref=must_excl_value, pred=pred_orig)
                case "one_of":
                    assert isinstance(value, list)
                    found = False
                    for one_of_value in value:
                        one_of_value = self.clean_answer(one_of_value)
                        if one_of_value in pred_orig:
                            found = True
                            break
                    score = score * found
                case "fuzzy_match":
                    ...
                    if value == "N/A":
                        ...
                        for reference in value:
                            score *= self.fuzzy_match(
                                ref=reference, pred=pred_orig, intent=intent
                            )
        return score
...
class StringSoftEvaluator(Evaluator):
    ...
    async def __call__(
        ...
        last_action = self.get_last_action(trajectory)
        pred = last_action["terminate_with_result"]
    ...
    async def __call__(
        ...
        for query in configs["eval"]["page_image_query"]:
            ...
            if target_url != "last":
                ...
                await asyncio.sleep(3)  # TODO[jykoh]: fix this hard-coded sleep
            ...
            for image in images:
                try:
                    ...
                    if not image_url.startswith(("http://", "https://", "www.")):  # type: ignore
                        image_url = urljoin(page.url, image_url)
                    image = Image.open(requests.get(image_url, stream=True).raw)  # type: ignore
                    ...
                    for exact_match_img in exact_match_imgs:
                        if exact_match_img.startswith("http"):
                            exact_match_pixels = Image.open(
                                requests.get(exact_match_img, stream=True).raw  # type: ignore
                            )
                        else:
